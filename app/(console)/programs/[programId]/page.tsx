      ...
      : program.status === "ENDED"
        ...
        : program.status === "QUEUED" || program.status === "PROCESSING"
          ...
          : !program.upstreamProgramId
            ...
            : undefined;
  const terminationReadiness =
    terminateDisabledReason ?? "Ready to submit to Yelp.";
  const programPostureItems = [
    {
      ...
      detail: program.upstreamProgramId
        ? "Local record has a confirmed Yelp program ID."
        : "Local record is waiting for upstream confirmation.",
    },
    {
      id: "budget",
      label: "Budget",
      status: scheduledBudgetDollars
        ? "SCHEDULED"
        : program.budgetCents
          ? "READY"
          : "UNKNOWN",
    ...
    },
    {
      ...
      detail: latestFeatureSnapshot
        ? `Last captured ${formatDateTime(latestFeatureSnapshot.capturedAt)}.`
        : "No feature snapshot captured yet.",
    ...
    },
    {
      ...
      value: latestJob
        ? titleCase(latestJob.type.toLowerCase().replaceAll("_", " "))
        : "No job",
      detail: latestJob
        ? `${formatDateTime(latestJob.createdAt)}${latestJob.upstreamJobId ? ` · ${latestJob.upstreamJobId}` : ""}`
        : "No Yelp operation recorded yet.",
    },
    {
      id: "business",
      label: "Business / location",
      status: program.business.location?.externalCrmLocationId
        ? "READY"
    ...
    },
    {
      ...
      status: businessOverride
        ? businessOverride.isEnabled
          ? "READY"
          : "INACTIVE"
        : "UNKNOWN",
      ...
      ? [
          {
            ...
            detail:
              "Review the job payload and retry only if the failure is safe to repeat.",
          },
        ]
      : []),
    ...(!program.upstreamProgramId &&
      ...
      ? [
          {
            ...
            detail:
              "Treat this program as unsettled until Yelp confirms the upstream program.",
          },
        ]
      : []),
    ...(capabilities.hasCrmIntegration &&
    !program.business.location?.externalCrmLocationId
      ? [
          {
            ...
            detail:
              "Reporting and lifecycle context may not line up cleanly with ServiceTitan.",
          },
        ]
      : []),
    ...
    <div>
      ...
      <Card className="mb-6 border-border/80 bg-muted/10 shadow-none">
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            Read this before changing budget, terminating, or trusting the
            program as settled upstream.
          </CardDescription>
        </CardHeader>
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Local status and upstream confirmation can diverge while Yelp is
                processing.
              </CardDescription>
            </CardHeader>
            <CardContent className="grid gap-3 text-sm md:grid-cols-2">
              ...
              <div>
                ...
                <div>
                  {formatCurrency(program.maxBidCents, program.currency)}
                </div>
              </div>
              <div>
              ...
              <div>
                ...
                <div>
                  {program.lastSyncedAt
                    ? formatDateTime(program.lastSyncedAt)
                    : "Not synced back from Yelp yet"}
                </div>
              </div>
              <div>
          ...
          {program.type === "CPC" ? (
            ...
            <Card>
              ...
              <CardContent className="text-sm text-muted-foreground">
                Budget operations are currently available for CPC programs only.
                Open a CPC program detail page to change current budget,
                schedule a future budget, or update pacing and max bid.
              </CardContent>
            </Card>
          )}
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Keep upstream state, local state, and audit notes separate.
              </CardDescription>
            </CardHeader>
              ...
              <div className="rounded-lg border border-border p-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium">
                    Program IDs, live status, accepted budget changes
                  </span>
                </div>
                <div className="mt-2 text-xs text-muted-foreground">
                  Final only after Yelp finishes the job.
                </div>
              </div>
              <div className="rounded-lg border border-border p-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium">
                    Drafted configuration, queue state, operator notes
                  </span>
                </div>
                <div className="mt-2 text-xs text-muted-foreground">
                  Saved immediately so operators can track intent.
                </div>
              </div>
              <div className="rounded-lg border border-border p-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium">
                    Terminate reason and requested end date
                  </span>
                </div>
                <div className="mt-2 text-xs text-muted-foreground">
                  Stored internally. Not sent to Yelp.
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Use this when the local record looks ahead of Yelp.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              {program.jobs.length === 0 ? (
                <div className="text-sm text-muted-foreground">
                  No jobs have been recorded for this program yet.
                </div>
              ) : (
                    ...
                    <div className="flex items-center justify-between gap-4">
                      <div>
                        ...
                        <div className="text-sm text-muted-foreground">
                          ...
                          {job.upstreamJobId ? `• ${job.upstreamJobId}` : ""}
                        </div>
                      </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                The last request state saved on the local record.
              </CardDescription>
            </CardHeader>
            <CardContent>
