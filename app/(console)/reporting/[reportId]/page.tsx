...
export default async function ReportDetailPage({
  ...
  const latestResult = [...report.results].sort(
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <Button asChild variant="outline">
            <a href={`/api/reports/${report.id}/export${currentQueryString}`}>
              Export CSV
            </a>
          </Button>
        }
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricCard
          title="Ad spend"
          value={formatCurrency(totals.adSpendCents ?? 0)}
        />
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Check freshness, scope, and source boundaries before treating
            ...
            <CardContent className="space-y-4 text-sm">
              <div className="flex flex-wrap gap-2">
                ...
                <Badge variant="secondary">
                  Internal outcomes stay separate
                </Badge>
              </div>
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <div>
                  ...
                  <div>
                    {formatDateTime(report.startDate, "MMM d, yyyy")} to{" "}
                    {formatDateTime(report.endDate, "MMM d, yyyy")}
                  </div>
                </div>
                <div>
                  ...
                  <div>
                    {latestResult
                      ? formatDateTime(latestResult.fetchedAt)
          ...
          <Card>
            <CardHeader>
              <CardTitle>
                {reportView.filters.view === "location"
                  ? "Location breakdown"
                  : "Service breakdown"}
              </CardTitle>
              <CardDescription>
                Yelp spend comes from the saved batch. Lead counts reflect Yelp
                intake in the selected window; booked and downstream outcomes
                are partner-derived.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-8">
                <MetricCard
                  title="Leads"
                  value={breakdown.totals.totalLeads}
                  description="Yelp leads created in the selected window."
                />
                <MetricCard
                  title="Mapped"
                  value={breakdown.totals.mappedLeads}
                  description="Leads with a resolved internal mapping."
                />
                <MetricCard
                  title="Active / contacted"
                  value={`${breakdown.totals.active} / ${breakdown.totals.contacted}`}
                  description="Partner lifecycle follow-through after intake."
                />
                <MetricCard
                  title="Booked / scheduled"
                  value={`${breakdown.totals.booked} / ${breakdown.totals.scheduled}`}
                  description="Partner lifecycle milestones from internal systems."
                />
                <MetricCard
                  title="In progress"
                  value={breakdown.totals.jobInProgress}
                  description="Current internal job-in-progress outcomes."
                />
                <MetricCard
                  title="Completed / won"
                  value={`${breakdown.totals.completed} / ${breakdown.totals.won}`}
                  description="Completed includes closed-won outcomes when present."
                />
                <MetricCard
                  title="Yelp spend"
                  value={formatCurrency(breakdown.totals.yelpSpendCents)}
                  description="Only where the saved Yelp batch can be mapped safely."
                />
                <MetricCard
                  title="CPL / win rate"
                  value={`${breakdown.totals.costPerLeadCents !== null ? formatCurrency(breakdown.totals.costPerLeadCents) : "—"} / ${breakdown.totals.winRate}%`}
                  description="Derived from Yelp spend plus partner lifecycle outcomes."
                />
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2 rounded-lg border border-border/80 bg-muted/15 px-4 py-3 text-xs text-muted-foreground">
                ...
                <Badge variant="outline">
                  Yelp spend is dimension-safe only
                </Badge>
                <span>
                  {reportView.filters.view === "service"
                      ...
                      <TableRow key={row.bucketId}>
                        <TableCell>
                          ...
                          <div className="text-xs text-muted-foreground">
                            {row.leadSharePct}% of leads • {row.spendSharePct}%
                            of spend
                          </div>
                        </TableCell>
                        <TableCell>
                          <div className="font-medium">
                            {formatCurrency(row.yelpSpendCents)}
                          </div>
                          <div className="text-xs text-muted-foreground">
                            Delayed Yelp batch spend
                          </div>
                        </TableCell>
                        <TableCell>
                          <div className="font-medium">
                          ...
                          <div className="text-xs text-muted-foreground">
                            {row.mappedLeads} mapped • {row.mappingRate}%
                            mapping rate
                          </div>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          <div className="space-y-1 text-xs text-muted-foreground">
                            <div>
                              <span className="font-medium text-foreground">
                                {row.costPerLeadCents !== null
                                  ? formatCurrency(row.costPerLeadCents)
                                  : "—"}
                              </span>{" "}
                              CPL
                            </div>
                            <div>
                              <span className="font-medium text-foreground">
                                {row.costPerBookedJobCents !== null
                                  ? formatCurrency(row.costPerBookedJobCents)
                                  : "—"}
                              </span>{" "}
                              cost per booked
                            </div>
                            <div>
                              <span className="font-medium text-foreground">
                                {row.costPerCompletedJobCents !== null
                                  ? formatCurrency(row.costPerCompletedJobCents)
                                  : "—"}
                              </span>{" "}
                              cost per completed
                            </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Raw combined Yelp rows remain available when the saved snapshot
                needs to move outside the console.
              </CardDescription>
            </CardHeader>
          ...
          {hasPermission(user.role.code, "diagnostics:read") ? (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Admin-only diagnostic view of the saved combined payload.
                </CardDescription>
              </CardHeader>
              <CardContent>
