...
import {
  ...
  ReportScheduleResendButton,
} from "@/components/forms/report-schedule-actions";
...
export default async function ReportingPage({
  ...
  const enabledSchedules = deliveryAdminState.schedules.filter(
    (schedule) => schedule.isEnabled,
  );
  const failedDeliveries = deliveryAdminState.recentRuns.filter(
    (run) => run.deliveryStatus === "FAILED",
  );
  const lastDeliveredRun = [...deliveryAdminState.recentRuns]
    .filter((run) => run.deliveredAt)
    .sort(
      (left, right) =>
        right.deliveredAt!.getTime() - left.deliveredAt!.getTime(),
    )[0];
  ...
  return (
    <div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard
          title="Ready"
          value={reports.summary.readyCount}
          description="Saved requests with at least one ready batch result."
        />
        <MetricCard
          title="Waiting on Yelp"
          value={reports.summary.pendingCount}
          description="Batch requests still being generated upstream."
        />
        <MetricCard
          title="Failed"
          value={reports.summary.failedCount}
          description="Requests that need a retry or credential review."
        />
        <MetricCard
          title="Last batch"
          value={
            latestSnapshotAt ? formatDateTime(latestSnapshotAt) : "Not yet"
          }
          description="Most recent stored Yelp reporting snapshot."
        />
      </div>
      ...
      <Card className="mt-6 border-border/70 bg-muted/10">
        <CardHeader>
          ...
          <CardDescription>
            Derived from CRM mapping and partner lifecycle records. These
            metrics are internal-only and do not come from Yelp reporting
            batches.
        ...
        <CardContent className="grid gap-3 md:grid-cols-3 xl:grid-cols-5">
          ...
          <div className="rounded-lg bg-background px-4 py-3">
            ...
            <div className="mt-1 text-lg font-semibold">
              {conversionMetrics.contactedLeads}
            </div>
          </div>
          <div className="rounded-lg bg-background px-4 py-3">
            <div className="text-xs uppercase tracking-[0.14em] text-muted-foreground">
          ...
          <div className="rounded-lg bg-background px-4 py-3">
            ...
            <div className="mt-1 text-lg font-semibold">
              {conversionMetrics.scheduledJobs}
            </div>
          </div>
          <div className="rounded-lg bg-background px-4 py-3">
            <div className="text-xs uppercase tracking-[0.14em] text-muted-foreground">
            ...
            <div className="mt-1 text-lg font-semibold">
              {conversionMetrics.jobInProgressJobs}
            </div>
          </div>
          <div className="rounded-lg bg-background px-4 py-3">
            <div className="text-xs uppercase tracking-[0.14em] text-muted-foreground">
            ...
            <div className="mt-1 text-lg font-semibold">
              {conversionMetrics.completedJobs}
            </div>
          </div>
          <div className="rounded-lg bg-background px-4 py-3">
            <div className="text-xs uppercase tracking-[0.14em] text-muted-foreground">
            ...
            <div className="mt-1 text-lg font-semibold">
              {conversionMetrics.wonLeads} / {conversionMetrics.lostLeads}
            </div>
          </div>
          <div className="rounded-lg bg-background px-4 py-3">
            <div className="text-xs uppercase tracking-[0.14em] text-muted-foreground">
      ...
      <div className="mt-6 space-y-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Weekly and monthly windows reuse the same saved reporting pipeline
              and deliver email with CSV attachments.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex flex-wrap items-center gap-2 rounded-lg border border-border/80 bg-muted/15 px-4 py-3 text-xs text-muted-foreground">
              <Badge>Operator delivery</Badge>
              <Badge
                variant={
                  deliveryAdminState.smtpConfigured ? "success" : "warning"
                }
              >
                  ...
                  {deliveryAdminState.schedules.map((schedule) => (
                    <TableRow key={schedule.id}>
                      ...
                      <TableCell>
                        ...
                        <div className="text-xs text-muted-foreground">
                          {schedule.deliveryScopeLabel}
                        </div>
                      </TableCell>
                      <TableCell>
                        <div>{schedule.defaultRecipientCount} default</div>
                        <div className="text-xs text-muted-foreground">
                          {schedule.locationOverrideCount > 0
                            ? `${schedule.locationOverrideCount} location override${schedule.locationOverrideCount === 1 ? "" : "s"}`
                            : "No location overrides"}
                        </div>
                      </TableCell>
                      <TableCell>
                        {schedule.lastSuccessfulGenerationAt
                          ? formatDateTime(schedule.lastSuccessfulGenerationAt)
                          : "Not yet"}
                      </TableCell>
                      <TableCell>
                        {schedule.lastSuccessfulDeliveryAt
                          ? formatDateTime(schedule.lastSuccessfulDeliveryAt)
                          : "Not yet"}
                      </TableCell>
                      <TableCell>
                        <StatusChip
                          status={schedule.isEnabled ? "ACTIVE" : "INACTIVE"}
                        />
                      </TableCell>
                      <TableCell>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard
          ...
          description="Recurring report schedules currently enabled."
        />
        <MetricCard
          title="Recent delivery runs"
          value={deliveryAdminState.recentRuns.length}
          description="Most recent account and location delivery records."
        />
        <MetricCard
          title="Failed deliveries"
          value={failedDeliveries.length}
          description="Runs that need SMTP or recipient review."
        />
        <MetricCard
          title="Last delivery"
          value={
            lastDeliveredRun?.deliveredAt
              ? formatDateTime(lastDeliveredRun.deliveredAt)
              : "Not yet"
          }
          description="Most recent successfully delivered recurring report."
        />
      </div>
      ...
      <Card className="mt-6">
        <CardHeader>
          ...
          <CardDescription>
            Review generation and send state separately. Generation depends on
            the Yelp batch finishing; delivery depends on SMTP and recipient
            validity.
          </CardDescription>
        </CardHeader>
          ...
          {deliveryAdminState.recentRuns.length === 0 ? (
            <div className="p-6">
              <EmptyState
                ...
                description="Recurring schedules create persisted delivery runs as soon as the current window is queued."
              />
            </div>
          ) : (
            <Table>
              ...
              <TableBody>
                {deliveryAdminState.recentRuns.map((run) => (
                  <TableRow key={run.id}>
                    ...
                    <TableCell>
                      {Array.isArray(run.recipientEmailsJson)
                        ? run.recipientEmailsJson.length
                        : 0}
                    </TableCell>
                    <TableCell>
                      <div className="flex flex-wrap gap-2">
                        {run.reportRequestId ? (
                          <Button asChild size="sm" variant="ghost">
                            <Link href={`/reporting/${run.reportRequestId}`}>
                              Open batch
                            </Link>
                          </Button>
                        ) : null}
      ...
      <Card className="mt-6">
        <CardHeader>
          ...
          <CardDescription>
            Review request timing, current status, and freshness before using a
          ...
          {reports.reports.length === 0 ? (
            <div className="p-6">
              <EmptyState
                ...
                description="Request the first daily or monthly Yelp batch report from the form above."
              />
            </div>
          ) : (
            <div className="space-y-4">
                ...
                <TableBody>
                  {reports.reports.map((report) => (
                    <TableRow key={report.id}>
                      ...
                      <TableCell>
                        {report.business?.name ?? "Multiple businesses"}
                      </TableCell>
                      ...
                      <TableCell>
                        {report.latestResultFetchedAt
                          ? formatDateTime(report.latestResultFetchedAt)
                          : "Still waiting on Yelp"}
                      </TableCell>
                      <TableCell>
                        <Link
              ...
              {reports.pagination.totalPages > 1 ? (
                <div className="flex flex-wrap items-center justify-between gap-3 px-4 pb-4 text-sm text-muted-foreground">
                  <span>
                    Page {reports.pagination.currentPage} of{" "}
                    {reports.pagination.totalPages} •{" "}
