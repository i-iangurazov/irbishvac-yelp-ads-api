...
function buildConnectorDefaults(
  overview: Awaited<ReturnType<typeof getServiceTitanConnectorOverview>>,
) {
  const environment: ServiceTitanEnvironment =
    overview.connector.environment === "INTEGRATION"
      ? "INTEGRATION"
      : "PRODUCTION";
      ...
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            <StatusChip
              status={overview.connector.enabled ? "ACTIVE" : "INACTIVE"}
            />
          </div>
        }
      />
        ...
        <CardContent className="grid gap-4 p-4 md:grid-cols-2 xl:grid-cols-4">
          <div className="space-y-1 xl:border-r xl:border-border/70 xl:pr-4">
            ...
            <div className="text-xs text-muted-foreground">
              {overview.connector.health?.detail ??
                "Save ServiceTitan credentials to enable testing and reference sync."}
            </div>
          </div>
          <div className="space-y-1 xl:border-r xl:border-border/70 xl:px-4">
            ...
            <div className="text-2xl font-semibold tracking-tight">
              {overview.counts.mappedLeads}
            </div>
            <div className="text-xs text-muted-foreground">
              {overview.counts.unresolvedLeadMappings} leads still need mapping
              or review
            </div>
          </div>
          <div className="space-y-1 xl:border-r xl:border-border/70 xl:px-4">
            ...
            <div className="text-2xl font-semibold tracking-tight">
              {formatDateTime(
                overview.lifecycle.latestSuccessfulRun?.finishedAt ?? null,
              )}
            </div>
            <div className="text-xs text-muted-foreground">
              Due now {overview.lifecycle.coverage.dueLeadCount} • Stale{" "}
              {overview.lifecycle.coverage.staleLeadCount}
            </div>
          </div>
          <div className="space-y-1 xl:pl-4">
            <div className="text-[11px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">
              Open connector issues
            </div>
            <div className="text-2xl font-semibold tracking-tight">
              {overview.counts.openConnectorIssues}
            </div>
            <Link
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Run safe reference refreshes and lifecycle reconciliation
                without rewriting Yelp history. Lifecycle sync appends partner
                lifecycle events only when ServiceTitan provides newer
                downstream evidence.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-3 rounded-lg border border-border/70 bg-muted/20 p-4">
                <div>
                  ...
                  <div className="mt-1 text-sm text-muted-foreground">
                    Poll mapped ServiceTitan leads and jobs, append newer
                    partner lifecycle events, and leave Yelp-native activity
                    untouched.
                  </div>
                </div>
                <ServiceTitanLifecycleSyncForm
                  disabled={!overview.connector.enabled}
                />
                <div className="grid gap-3 text-sm text-muted-foreground md:grid-cols-3">
                  <div className="rounded-lg border border-border/70 bg-background p-4">
                    <div className="font-medium text-foreground">
                      Pollable mapped leads
                    </div>
                    <div className="mt-1">
                      {overview.lifecycle.coverage.pollableLeadCount}
                    </div>
                  </div>
                  <div className="rounded-lg border border-border/70 bg-background p-4">
                    <div className="font-medium text-foreground">
                      Manual-only mappings
                    </div>
                    <div className="mt-1">
                      {overview.lifecycle.coverage.manualOnlyMappedLeadCount}
                    </div>
                  </div>
                  <div className="rounded-lg border border-border/70 bg-background p-4">
                    <div className="font-medium text-foreground">
              ...
              <div className="space-y-3 rounded-lg border border-border/70 bg-muted/20 p-4">
                <div>
                  ...
                  <div className="mt-1 text-sm text-muted-foreground">
                    Refresh the ServiceTitan business-unit and category catalogs
                    used by the mapping tables below. These are safe
                    reference-data syncs, not destructive lead rewrites.
                  </div>
                </div>
                <ServiceTitanReferenceSyncForm
                  disabled={!overview.connector.enabled}
                />
              </div>
              <div className="grid gap-3 text-sm text-muted-foreground md:grid-cols-2">
                <div className="rounded-lg border border-border/70 bg-muted/20 p-4">
                  ...
                  <div className="mt-1">
                    {formatCatalogStatus(
                      overview.catalog.businessUnits.totalCount,
                      overview.catalog.businessUnits.hasMore,
                    )}
                  </div>
                  <div className="mt-1">
                    Last refreshed{" "}
                    {formatDateTime(
                      overview.catalog.businessUnits.syncedAt
                        ? new Date(overview.catalog.businessUnits.syncedAt)
                        : null,
                    )}
                  </div>
                </div>
                <div className="rounded-lg border border-border/70 bg-muted/20 p-4">
                  ...
                  <div className="mt-1">
                    {formatCatalogStatus(
                      overview.catalog.categories.totalCount,
                      overview.catalog.categories.hasMore,
                    )}
                  </div>
                  <div className="mt-1">
                    Last refreshed{" "}
                    {formatDateTime(
                      overview.catalog.categories.syncedAt
                        ? new Date(overview.catalog.categories.syncedAt)
                        : null,
                    )}
                  </div>
                </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Assign each saved Yelp business to the internal location that
                should own downstream lifecycle and reporting rollups.
              </CardDescription>
            </CardHeader>
            <CardContent className="p-0">
              {overview.businesses.length === 0 ? (
                <div className="p-6">
                  <EmptyState
                    title="No saved Yelp businesses yet"
                    description="Save businesses first so the connector can route downstream data to internal locations."
                  />
                </div>
              ) : (
                <Table>
                      ...
                      <TableRow key={business.id}>
                        ...
                        <TableCell>
                          <ConnectorBusinessLocationForm
                            ...
                            locations={overview.locations.map((location) => ({
                              id: location.id,
                              name: location.name,
                            }))}
                          />
                        ...
                        <TableCell>
                          {business.locationConnectorReferenceName ? (
                            <div>
                              ...
                              <div className="text-xs text-muted-foreground">
                                {business.locationConnectorReferenceId}
                              </div>
                            </div>
                          ) : (
                            <StatusChip
                              status={
                                business.needsAttention ? "UNMAPPED" : "UNKNOWN"
                              }
                            />
                          )}
                        </TableCell>
          ...
          <Card>
            <CardHeader>
              <CardTitle>
                Internal locations to ServiceTitan references
              </CardTitle>
              <CardDescription>
                Map each internal location to the ServiceTitan business unit or
                branch reference used by your downstream workflow.
              </CardDescription>
            </CardHeader>
            <CardContent className="p-0">
              {overview.locations.length === 0 ? (
                <div className="p-6">
                  <EmptyState
                    title="No internal locations yet"
                    description="Add internal locations before linking them to ServiceTitan references."
                  />
                </div>
              ) : (
                <Table>
                        ...
                        <TableCell>
                          <div className="space-y-2">
                            <ConnectorLocationReferenceForm
                              ...
                              defaultReferenceId={
                                location.externalCrmLocationId
                              }
                              options={overview.catalog.businessUnits.rows.map(
                                (row) => ({ id: row.id, name: row.name }),
                              )}
                            />
                            {location.connectorReferenceName ? (
                              <div className="text-xs text-muted-foreground">
                                Linked to {location.connectorReferenceName} (
                                {location.externalCrmLocationId})
                              </div>
                            ) : (
                              <div className="text-xs text-amber-700">
                                No ServiceTitan reference linked yet.
                              </div>
                            )}
                          </div>
                        </TableCell>
          ...
          <Card>
            <CardHeader>
              <CardTitle>
                Service categories to ServiceTitan categories
              </CardTitle>
              <CardDescription>
                Store the ServiceTitan category IDs or names used for downstream
                service mapping. Unknown categories stay visible until they are
                mapped.
              </CardDescription>
            </CardHeader>
            <CardContent className="p-0">
              {overview.serviceCategories.length === 0 ? (
                <div className="p-6">
                  <EmptyState
                    ...
                    description="Create normalized service categories before connector-backed service mapping can be managed here."
                  />
                </div>
              ) : (
                <Table>
                    ...
                    {overview.serviceCategories.map((serviceCategory) => (
                      <TableRow key={serviceCategory.id}>
                        ...
                        <TableCell>
                          ...
                          <div className="text-xs text-muted-foreground">
                            {serviceCategory.reportingSnapshotCount} report rows
                          </div>
                        </TableCell>
                      </TableRow>
                    ))}
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Failed connector syncs, unmapped leads, mapping conflicts, and
                stale lifecycle coverage stay in the main operator queue.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              {overview.openIssues.length === 0 ? (
                <EmptyState
                  title="No open connector issues"
                  description="Connector-specific issues will appear here and in Audit when sync, mapping, or lifecycle coverage breaks."
                />
              ) : (
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Reference refreshes and downstream sync runs share the same
                audit and retry pattern as the rest of the product.
              </CardDescription>
            </CardHeader>
            <CardContent className="p-0">
              {overview.recentSyncRuns.length === 0 ? (
                <div className="p-6">
                  <EmptyState
                    title="No connector syncs recorded yet"
                    description="Run a ServiceTitan reference sync or downstream lifecycle sync to start populating this history."
                  />
                </div>
              ) : (
                <Table>
