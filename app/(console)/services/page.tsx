    ...
    <div>
      ...
      <Card className="border-border/70 bg-muted/20">
        <CardHeader>
          ...
          <CardDescription>
            This page exists to keep the data model visible without pretending
            per-service operations are already in the MVP.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-2 text-sm text-muted-foreground">
          <div>
            {overview.counts.serviceCategories} normalized services are stored
            today.
          </div>
          <div>
            {overview.counts.classifiedLeads} leads and{" "}
            {overview.counts.reportingSnapshots} reporting snapshots are already
            available for future service-level rollups.
          </div>
          <div>
            Service mapping and related sync issues remain internal foundation
            work for now.
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="mt-6">
        <CardHeader>
          ...
          <CardDescription>
            Rules stay normalized here so Yelp labels and CRM labels do not leak
            into each other.
          </CardDescription>
        </CardHeader>
              ...
              <TableBody>
                {overview.serviceCategories.map((serviceCategory) => (
                  <TableRow key={serviceCategory.id}>
                    ...
                    <TableCell>
                      {serviceCategory._count.yelpReportingSnapshots}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
