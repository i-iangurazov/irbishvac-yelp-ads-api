...
export default async function DashboardPage() {
  ...
  const credentialBlockers = credentialHealth.filter(
    ({ health }) =>
      health.setupLabel !== "Credentials saved" ||
  ...
  return (
    <div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        <MetricCard
          title="Launch-ready businesses"
          value={launchReadyBusinesses.length}
          description="Saved businesses that can move directly into CPC creation."
          icon={<SearchCheck className="h-5 w-5 text-muted-foreground" />}
        />
        <MetricCard
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              What is live right now, what is blocked, and where to fix it.
            </CardDescription>
          </CardHeader>
            ...
            <div className="rounded-lg border border-border bg-muted/40 p-4">
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {enabledApis.length > 0 ? (
                  enabledApis.map((label) => <Badge key={label}>{label}</Badge>)
                ) : (
                  <Badge variant="outline">None enabled yet</Badge>
                )}
              </div>
              {settings.capabilities.demoModeEnabled ? (
                <div className="mt-3 text-xs text-muted-foreground">
                  Demo mode is on. Some flows may complete locally without a
                  live Yelp request.
                </div>
              ) : null}
            </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Jump straight into the next useful action.
            </CardDescription>
          </CardHeader>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Requests that need follow-up before operators trust the local
              state.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            {failedJobs.length === 0 ? (
              <EmptyState
                ...
                description="Recent Yelp jobs completed successfully or none have been submitted yet."
              />
            ) : (
              failedJobs.slice(0, 6).map((job) => (
                ...
                >
                  <div>
                    ...
                    <div className="text-sm text-muted-foreground">
                      {job.upstreamJobId ?? "No upstream job ID yet"}
                    </div>
                  </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Clear these before launching new CPC work.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            {readinessIssues.length === 0 ? (
              <EmptyState
                title="All clear"
                description="Saved businesses currently meet the baseline readiness checks for CPC."
              />
            ) : (
      ...
      <Card className="mt-6">
        <CardHeader>
          ...
          <CardDescription>
            Saved Yelp batches. Pending runs are delayed snapshots, not live
            reporting.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
