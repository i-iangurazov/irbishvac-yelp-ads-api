    ...
    <div>
      ...
      <Card className="mb-6 border-border/80 bg-muted/10 shadow-none">
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            Use this before enabling automation, changing programs, or trusting
        ...
        <div className="space-y-6">
            ...
            <CardHeader>
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <CardDescription>
                    Use this before adding the business to live lead intake or
                    autoresponder scope.
                  </CardDescription>
                </div>
            ...
            <CardContent className="space-y-4">
              <div className="rounded-lg border border-border bg-muted/30 p-3 text-sm">
                <div className="font-medium">
                  {business.yelpLeadOnboarding.label}
                </div>
                <div className="mt-1 text-muted-foreground">
                  {business.yelpLeadOnboarding.detail}
                </div>
                <div className="mt-2 text-xs font-medium uppercase tracking-[0.14em] text-muted-foreground">
                  Next action
                </div>
                <div className="mt-1">
                  {business.yelpLeadOnboarding.nextAction}
                </div>
              </div>
              {canManageBusinesses ? (
                <div className="flex flex-wrap gap-2">
                  <BusinessYelpLeadsCheckButton
                    businessId={business.id}
              ...
              <div className="rounded-lg border border-border/80 bg-background">
                <div className="border-b border-border px-4 py-3">
                  ...
                  <div className="text-xs text-muted-foreground">
                    Last recorded evidence for intake, sync, and thread
                    delivery.
                  </div>
                </div>
                <div className="divide-y divide-border">
                  {business.yelpConnectionProofTrail.map((proof) => (
                    <div
                      className="grid gap-3 px-4 py-3 text-sm md:grid-cols-[180px_1fr_auto]"
                      key={proof.id}
                    >
                      <div>
                        ...
                        <div className="mt-1 text-xs text-muted-foreground">
                          {proof.occurredAt
                            ? formatDateTime(proof.occurredAt)
                            : "No timestamp"}
                        </div>
                      </div>
                      <div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Use this before submitting a new CPC request.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              <StatusChip
                status={business.readiness.isReadyForCpc ? "READY" : "FAILED"}
              />
                ...
                <div className="mt-1 text-muted-foreground">
                  {business.readiness.isReadyForCpc
                    ...
                    : (business.readiness.missingItems[0] ??
                      "Review the saved business inputs before launching.")}
                </div>
              </div>
              <ul className="space-y-2 text-sm text-muted-foreground">
                {business.readiness.missingItems.length === 0 ? (
                  ...
                  business.readiness.missingItems.map((item) => (
                    <li key={item}>{item}</li>
                  ))
                )}
              </ul>
          ...
          <Card>
            ...
            <CardContent className="space-y-3 text-sm">
              ...
              <div>
                <div className="text-muted-foreground">Yelp business sync</div>
                <div className="mt-1 flex flex-wrap items-center gap-2">
                  <StatusChip
                    status={business.readiness.yelpBusinessSyncStatus}
                  />
                  <span>{business.readiness.yelpBusinessSyncDetail}</span>
                </div>
                {business.readiness
              ...
              <div>
                ...
                <div>
                  {business.operationalSummary.counts.leads} leads ·{" "}
                  {business.operationalSummary.counts.programs} programs ·{" "}
              ...
              <div>
                ...
                <div>
                  {business.categories.length > 0
                    ? business.categories.map(formatYelpCategory).join(", ")
                    : "No categories saved"}
                </div>
                {business.categories.some((category) => !category.alias) ? (
                  <div className="mt-1 text-xs text-warning">
                    Some saved categories are missing Yelp aliases. CPC programs
                    require alias-backed categories.
                  </div>
                ) : null}
              </div>
              <div>
                ...
                {business.readiness.adsEligibilityMessage ? (
                  ...
                  <div className="mt-1 text-xs text-muted-foreground">
                    Yelp ad eligibility is unknown until Yelp accepts or rejects
                    an ads operation for this business.
                  </div>
                ) : null}
              </div>
            </CardContent>
        ...
        <div className="space-y-6">
            ...
            <CardContent className="p-0">
              {business.liveProgramInventory.programs.length > 0 ? (
                      ...
                      <TableRow key={program.program_id}>
                        ...
                        <TableCell>
                          ...
                          {program.program_pause_status ? (
                            <div className="mt-1 text-xs text-muted-foreground">
                              {titleCase(program.program_pause_status)}
                            </div>
                          ) : null}
                        </TableCell>
                          ...
                          <div className="flex flex-wrap gap-1">
                            {program.active_features.length > 0 ? (
                              program.active_features.map((feature) => (
                                <Badge
                                  key={`${program.program_id}-${feature}`}
                                  variant="success"
                                >
                                  {titleCase(feature)}
                                </Badge>
                          ...
                          {program.localProgramId ? (
                            <div className="space-y-1">
                              <Link
                                ...
                                href={`/programs/${program.localProgramId}`}
                              >
                                Open local program
                              </Link>
                              {program.localProgramStatus ? (
                                <StatusChip
                                  status={program.localProgramStatus}
                                />
                              ) : null}
                            </div>
                          ) : (
                ...
                <div className="space-y-2 p-6 text-sm text-muted-foreground">
                  <div>
                    No live Yelp program inventory is available for this
                    business yet.
                  </div>
                  {business.liveProgramInventory.message ? (
                    <div>{business.liveProgramInventory.message}</div>
                  ) : null}
                </div>
              )}
            </CardContent>
            ...
            <CardContent className="p-0">
              {business.currentPrograms.length > 0 ? (
                          ...
                          <div className="space-y-1">
                            ...
                            <div className="text-xs text-muted-foreground">
                              {program.upstreamProgramId
                                ? `Yelp ID ${program.upstreamProgramId}`
                                : "No confirmed Yelp ID"}
                            </div>
                          </div>
                        </TableCell>
                      </TableRow>
                ...
                <div className="space-y-3 p-6 text-sm text-muted-foreground">
                  <div>
                    No active local programs are currently tracked for this
                    business.
                  </div>
                  <Link
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Saved Yelp batch requests for this business.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              ...
              {business.reportRequests.length === 0 ? (
                <div className="text-sm text-muted-foreground">
                  No report requests have been saved for this business yet.
                </div>
              ) : null}
            </CardContent>
          </Card>
