...
- Live readiness: **9 / 10 mandatory live gates (90 / 100)**
- Release branch: `main`
- Baseline commit: `6456fdf`
- Latest application checkpoint: `bb0f66c`
- Live blocker: the deployed Vercel Production runtime does not have an Anthropic connection, so deployed onboarding now correctly blocks activation until a platform administrator configures it and redeploys.

`Ready` still requires every mandatory release gate. The weighted implementation/verification score can be complete while a deployment-owned live gate remains blocked.
...
| Atomic requirement                  | Points | Earned | Evidence                                                                                                |
| ----------------------------------- | -----: | -----: | ------------------------------------------------------------------------------------------------------- |
| Full unit/integration/browser suite |      1 |      1 | 96 files / 380 tests and 6 Playwright flows passed                                                      |
...
| Anthropic configured in deployed Vercel runtime          | Blocked | Authenticated production UI reports the connection is not configured |
...
Yelp authentication, production deployment, migrations, lead synchronization, queued-webhook processing, provider-backed Claude generation, safe fallback policy, exactly-once Yelp delivery, audit/usage persistence, and Yelp read-back are live-verified.

The only remaining live gate is deployment configuration: add `ANTHROPIC_API_KEY` to the Vercel project's **Production** environment, keep `CLAUDE_REPLY_MODEL=claude-opus-4-6`, and redeploy. Until the authenticated production UI confirms the Anthropic connection and onboarding can pass that gate, the product remains **Controlled Pilot Only** rather than ready for unrestricted external-client onboarding.
