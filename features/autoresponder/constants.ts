...
export const leadAutomationScopeModeValues = [
  "ALL_BUSINESSES",
  "SELECTED_BUSINESSES",
] as const;
export type LeadAutomationScopeModeValue =
  (typeof leadAutomationScopeModeValues)[number];
...
export type LeadConversationAutomationModeValue =
  (typeof leadConversationAutomationModeValues)[number];
...
export const leadConversationAutomationModeOptions = [
  {
    ...
    label: "Review-only",
    description:
      "Generate a suggested reply, but require a person to review and send it.",
  },
  {
    value: leadConversationAutomationModeValues[1],
    label: "Bounded auto-reply",
    description:
      "Auto-send only for approved low-risk intents. Everything else stops or falls back.",
  },
  {
    value: leadConversationAutomationModeValues[2],
    label: "Human handoff",
    description:
      "Do not auto-send. Route new inbound conversation turns to a person.",
  },
] as const;
...
export type LeadConversationIntentValue =
  (typeof leadConversationIntentValues)[number];
...
export const leadConversationIntentOptions = [
  {
    ...
    description:
      "Customer replied with photos, address, symptoms, or other useful details.",
  },
  {
    value: leadConversationIntentValues[1],
    label: "Basic acknowledgment",
    description:
      "Simple thank-you or confirmation that does not need risky business claims.",
  },
  {
    value: leadConversationIntentValues[2],
    label: "Simple clarification",
    description:
      "Safe next-step clarification without pricing or scheduling promises.",
  },
  {
    value: leadConversationIntentValues[3],
    label: "Booking intent",
    description:
      "Customer wants to book or move forward, but the business should usually review.",
  },
  {
    value: leadConversationIntentValues[4],
    label: "Quote or pricing",
    description:
      "Price, estimate, or quote request. Keep this out of auto-send.",
  },
  {
    value: leadConversationIntentValues[5],
    label: "Availability or timing",
    description:
      "Arrival, scheduling window, or availability request. Keep this out of auto-send.",
  },
  {
...
export const leadAutomationScopeModeOptions = [
  {
    ...
    description:
      "Use the tenant default for every business that does not have its own override.",
  },
  {
    value: leadAutomationScopeModeValues[1],
    label: "Selected businesses only",
    description:
      "Only businesses in the selected list use the tenant default. All others stay off unless they have an override.",
  },
] as const;

export const approvedLeadAiModelValues = [
  "claude-haiku-4-5",
  "claude-sonnet-4-6",
  "claude-opus-4-6",
] as const;
export type ApprovedLeadAiModelValue =
  (typeof approvedLeadAiModelValues)[number];

export const leadAutomationRenderModeValues = [
  "STATIC",
  "AI_ASSISTED",
] as const;
export type LeadAutomationRenderModeValue =
  (typeof leadAutomationRenderModeValues)[number];
...
export const leadAutomationRenderModeOptions = [
  ...
  },
  {
    ...
    label: "AI-assisted",
    description:
      "Use AI guidance to draft the message, with the saved template as the fallback.",
  },
] as const;
...
export const approvedLeadAiModelOptions = [
  {
    ...
    label: "Economy",
    description:
      "$1 / 1M input tokens and $5 / 1M output tokens. Best for high-volume, straightforward replies.",
  ...
  },
  {
    ...
    description:
      "$3 / 1M input tokens and $15 / 1M output tokens. Recommended production balance.",
  ...
  },
  {
    ...
    label: "Premium",
    description:
      "$5 / 1M input tokens and $25 / 1M output tokens. Highest-quality option for complex conversations.",
...
export const defaultLeadAiModel = approvedLeadAiModelOptions[1].value;
export const defaultLeadAiMonthlyBudgetUsd = 50;
export const defaultLeadAiMonthlyMessageLimit = 5_000;
export const defaultLeadAiMonthlyTokenLimit = 5_000_000;
export const defaultLeadAiUsageWarningPercent = 80;

export const leadAutomationCadenceValues = [
  "INITIAL",
  "FOLLOW_UP_24H",
  "FOLLOW_UP_7D",
] as const;
