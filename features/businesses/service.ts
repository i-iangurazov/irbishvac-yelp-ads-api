...
import {
  ensureYelpAccess,
  ensureYelpBusinessSubscriptionsAccess,
...
import { YelpPartnerSupportClient } from "@/lib/yelp/partner-support-client";
import {
  yelpBusinessMatchResponseSchema,
  type YelpUpstreamProgramDto,
...
type ReadinessState = {
  ...
  yelpBusinessSyncStatus:
    ...
    | "NO_ACCESS"
    | "ERROR";
  yelpBusinessSyncCheckedAt: string | null;
  yelpBusinessSyncSource: string | null;
  yelpBusinessSyncDetail: string | null;
  yelpBusinessSyncDestinationBusinessId: string | null;
  yelpBusinessSyncDestinationLocalBusinessId: string | null;
};
...
function sortProgramsByMostRecentActivity(programs: YelpUpstreamProgramDto[]) {
  return [...programs].sort((left, right) => {
    const leftDate =
      left.end_date && left.end_date !== "9999-12-31"
        ? left.end_date
        : (left.start_date ?? "");
    const rightDate =
      right.end_date && right.end_date !== "9999-12-31"
        ? right.end_date
        : (right.start_date ?? "");
    ...
    const statusRank =
      (liveProgramStatusOrder.get(left.program_status) ?? 99) -
      (liveProgramStatusOrder.get(right.program_status) ?? 99);
...
function buildYelpBusinessSyncState(readinessJson: unknown) {
  const readiness = asReadinessRecord(readinessJson);
  const rawStatus = readStringValue(readiness, "yelpBusinessSyncStatus");
  const status: ReadinessState["yelpBusinessSyncStatus"] =
    rawStatus === "ACTIVE" ||
    rawStatus === "MIGRATED" ||
    rawStatus === "NOT_FOUND" ||
    rawStatus === "NO_ACCESS" ||
    rawStatus === "ERROR"
      ? rawStatus
      : "UNKNOWN";
  const destinationBusinessId = readStringValue(
    readiness,
    "yelpBusinessSyncDestinationBusinessId",
  );
  const errorMessage = readStringValue(
    readiness,
    "yelpBusinessSyncErrorMessage",
  );
  const detail =
    status === "ACTIVE"
      ? "Yelp still recognizes this business ID."
      : status === "MIGRATED"
        ? destinationBusinessId
          ? `Yelp says this business migrated to ${destinationBusinessId}.`
          : "Yelp says this business migrated to another business."
        : status === "NOT_FOUND"
          ? "Yelp did not recognize this business ID at the last sync."
          : status === "NO_ACCESS"
            ? "The token could not access this business; it may be unclaimed, non-advertising, or outside the account."
            : status === "ERROR"
              ? (errorMessage ?? "The last Yelp business sync failed.")
              : "Business existence has not been checked yet.";

  return {
    status,
    checkedAt: readStringValue(readiness, "yelpBusinessSyncCheckedAt"),
    source: readStringValue(readiness, "yelpBusinessSyncSource"),
    detail,
    destinationBusinessId,
    destinationLocalBusinessId: readStringValue(
      readiness,
      "yelpBusinessSyncDestinationLocalBusinessId",
    ),
    mergedBusinessIds: readStringArrayValue(
      readiness,
      "yelpBusinessSyncMergedBusinessIds",
    ),
    errorCode: readStringValue(readiness, "yelpBusinessSyncErrorCode"),
    errorMessage,
  };
}

function summarizeReportRecipients(
  schedule: Awaited<
    ReturnType<typeof getBusinessById>
  >["reportSchedules"][number],
) {
  const accountRecipients = countJsonArray(schedule.recipientEmailsJson);
  const locationOverrides = countJsonArray(
    schedule.locationRecipientOverridesJson,
  );
  const recipientLabel =
    accountRecipients === 1
      ? "1 account recipient"
      : `${accountRecipients} account recipients`;

  return locationOverrides > 0
    ? `${recipientLabel}, ${locationOverrides} location routes`
    : recipientLabel;
}
...
function buildBusinessOperationalSummary(params: {
  ...
  const override =
    automationScope.override ?? business.leadAutomationOverrides[0] ?? null;
  const effectiveAutomation = automationScope.effectiveSettings;
  const activeReportSchedules = business.reportSchedules.filter(
    (schedule) => schedule.isEnabled,
  );
  const openIssues = business.operatorIssues;
  const activePrograms = currentPrograms.filter(
    (program) => program.status === "ACTIVE" || program.status === "SCHEDULED",
  );
  const inFlightPrograms = currentPrograms.filter(
    (program) => program.status === "QUEUED" || program.status === "PROCESSING",
  );
  const failedPrograms = currentPrograms.filter(
    (program) => program.status === "FAILED" || program.status === "PARTIAL",
  );
  const latestSync =
    business.syncRuns.find(
      (run) =>
  ...
  const yelpProofAt =
    ...
    latestLead?.latestInteractionAt ??
    null;
  ...
  const conversationStatus = effectiveAutomation.conversationAutomationEnabled
    ? effectiveAutomation.conversationMode
    : "INACTIVE";
  const programStatus =
    failedPrograms.length > 0
      ? "FAILED"
      : inFlightPrograms.length > 0
        ? "PROCESSING"
        : activePrograms.length > 0
          ? "ACTIVE"
          : "INACTIVE";
  const connectorStatus = !capabilities.hasCrmIntegration
    ? "INACTIVE"
  ...
  const items: OperationalPostureItem[] = [
    ...
    },
    {
      ...
      value: effectiveAutomation.conversationAutomationEnabled
        ? effectiveAutomation.conversationMode.replaceAll("_", " ")
        : "Off",
    ...
    },
    {
      ...
      href: currentPrograms[0]
        ? `/programs/${currentPrograms[0].id}`
        : `/programs/new?businessId=${business.id}`,
    },
    {
      id: "servicetitan",
    ...
    },
    {
      ...
      value:
        activeReportSchedules.length > 0
          ? `${activeReportSchedules.length} enabled`
          : "Not scheduled",
      detail: reportDetail,
      href: "/reporting",
    },
    {
      ...
      detail:
        openIssues[0]?.title ?? "No open operator issues for this business.",
      href: "/audit",
    },
  ];
  ...
  if (
    business._count.yelpLeads > 0 &&
    !override &&
    !automationScope.defaultsApplyToBusiness
  ) {
    warnings.push({
      ...
      detail:
        "This business has Yelp leads, but effective autoresponder settings do not cover it.",
      href: "/autoresponder",
    });
  }
  ...
  if (effectiveAutomation.isEnabled && yelpConnectionStatus !== "READY") {
    warnings.push({
      ...
      detail:
        "Confirm webhook or backfill proof before relying on automatic replies.",
      href: "/autoresponder",
    });
  }
  ...
  if (
    ...
    warnings.push({
      ...
      detail:
        "Leads can arrive, but downstream lifecycle sync may not route cleanly.",
      href: "/integrations",
    });
  }
  ...
  if (
    currentPrograms.some(
      (program) => program.status !== "DRAFT" && !program.upstreamProgramId,
    )
  ) {
    warnings.push({
      ...
      detail:
        "Open the program and review the latest Yelp job before treating it as settled.",
    });
  }
  ...
  if (latestSync?.status === "FAILED" || latestSync?.status === "PARTIAL") {
    warnings.push({
      ...
      detail:
        ...
        `${latestSync.type} ended with ${latestSync.status.toLowerCase()}.`,
      href: "/audit",
    });
  }
...
function buildYelpConnectionProofTrail(params: {
  ...
  latestLeadSyncRun:
    | Awaited<ReturnType<typeof getBusinessById>>["syncRuns"][number]
    | null;
}): YelpConnectionProof[] {
  ...
  const leadReadStatus = readStringValue(
    readiness,
    "yelpLeadReadinessCheckStatus",
  );
  const leadReadCheckedAt = readDateValue(
    readiness,
    "yelpLeadReadinessCheckedAt",
  );
  const leadReadCorrelationId = readStringValue(
    readiness,
    "yelpLeadReadinessCheckCorrelationId",
  );
  const leadReadReturnedLeadIds = readNumberValue(
    readiness,
    "yelpLeadReadinessReturnedLeadIds",
  );
  const leadReadHasMore = readBooleanValue(
    readiness,
    "yelpLeadReadinessHasMore",
  );
  const leadReadErrorMessage = readStringValue(
    readiness,
    "yelpLeadReadinessCheckErrorMessage",
  );
  const subscriptionRequestedAt = readDateValue(
    readiness,
    "yelpLeadSubscriptionRequestedAt",
  );
  const subscriptionVerifiedAt = readDateValue(
    readiness,
    "yelpLeadSubscriptionVerifiedAt",
  );
  const subscriptionStatus = readStringValue(
    readiness,
    "yelpLeadSubscriptionStatus",
  );
  const subscriptionRequestCorrelationId = readStringValue(
    readiness,
    "yelpLeadSubscriptionRequestCorrelationId",
  );
  const subscriptionVerificationCorrelationId = readStringValue(
    readiness,
    "yelpLeadSubscriptionVerificationCorrelationId",
  );
  const subscribedAt = readDateValue(
    readiness,
    "yelpLeadSubscriptionSubscribedAt",
  );
  const webhookProofAt = latestLead?.latestWebhookReceivedAt ?? null;
  const reconcileProofAt =
    latestLeadSyncRun?.lastSuccessfulSyncAt ??
    latestLeadSyncRun?.finishedAt ??
    latestLead?.lastSyncedAt ??
    null;
  const sendProofAt =
    latestSend?.completedAt ?? latestSend?.triggeredAt ?? null;
  ...
  return [
    ...
    },
    {
      ...
      occurredAt:
        subscriptionStatus === "CONFIRMED"
          ? (subscribedAt ?? subscriptionVerifiedAt)
    ...
    },
    {
      ...
      status:
        latestLeadSyncRun?.status ??
        (latestLead?.lastSyncedAt ? "READY" : "UNKNOWN"),
...
export function buildCpcReadiness(
  readinessJson: unknown,
  categoriesJson: unknown,
): ReadinessState {
  const categories = normalizeYelpCategories(categoriesJson);
  const readiness =
    typeof readinessJson === "object" && readinessJson !== null
      ? readinessJson
      : {};
  const yelpBusinessSync = buildYelpBusinessSyncState(readinessJson);
  const persistedEligibilityStatus =
    typeof (readiness as Record<string, unknown>).adsEligibilityStatus ===
    "string"
      ? ((readiness as Record<string, unknown>).adsEligibilityStatus as string)
      : undefined;
  const adsEligibilityBlocked =
    Boolean((readiness as Record<string, unknown>).adsEligibilityBlocked) ||
    persistedEligibilityStatus === "INELIGIBLE";
  const adsEligibilityMessage =
    typeof (readiness as Record<string, unknown>).adsEligibilityMessage ===
    "string"
      ? ((readiness as Record<string, unknown>).adsEligibilityMessage as string)
      : undefined;
  const adsEligibilityStatus =
    persistedEligibilityStatus === "ELIGIBLE"
      ? "ELIGIBLE"
      : adsEligibilityBlocked
        ? "BLOCKED"
        : "UNKNOWN";
  const hasAboutText = Boolean(
    (readiness as Record<string, unknown>).hasAboutText ??
    (readiness as Record<string, unknown>).aboutThisBusiness,
  );
  const hasCategories =
    categories.length > 0 ||
    Boolean((readiness as Record<string, unknown>).hasCategories);
  const missingItems = [
    ...(yelpBusinessSync.status === "MIGRATED"
      ? [
          yelpBusinessSync.destinationBusinessId
            ? `Yelp migrated this business ID to ${yelpBusinessSync.destinationBusinessId}. Use the destination business before launching.`
            : "Yelp marked this business ID as migrated. Use the destination business before launching.",
        ]
      : []),
    ...(yelpBusinessSync.status === "NOT_FOUND"
      ? ["Yelp did not recognize this business ID at the last business sync."]
      : []),
    ...(adsEligibilityBlocked
      ? [
          adsEligibilityMessage ??
            "This business is not eligible for Yelp Ads because Yelp marked it as an advertising-restricted category.",
        ]
      : []),
  ...
  return {
    ...
    adsEligibilityMessage,
    yelpBusinessSyncStatus: yelpBusinessSync.status,
    yelpBusinessSyncCheckedAt: yelpBusinessSync.checkedAt,
    yelpBusinessSyncSource: yelpBusinessSync.source,
    yelpBusinessSyncDetail: yelpBusinessSync.detail,
    yelpBusinessSyncDestinationBusinessId:
      yelpBusinessSync.destinationBusinessId,
    yelpBusinessSyncDestinationLocalBusinessId:
      yelpBusinessSync.destinationLocalBusinessId,
  };
}

export async function getBusinessesIndex(tenantId: string, search?: string) {
  const [businesses, capabilities] = await Promise.all([
    listBusinesses(tenantId, search),
...
export async function getBusinessDetail(tenantId: string, businessId: string) {
  const [business, deleteImpact] = await Promise.all([
    getBusinessById(businessId, tenantId),
    getBusinessDeleteImpact(businessId, tenantId),
  ]);
  ...
  const currentPrograms = business.programs.filter(
    (program: (typeof business.programs)[number]) =>
  ...
  const forwarderAllowlist = buildYelpForwarderAllowlistState({
    ...
    allowedBusinessIds: parseYelpAllowedBusinessIds(
      getServerEnv().YELP_ALLOWED_BUSINESS_IDS,
    ),
  });
  ...
  const yelpLeadOnboarding = buildYelpLeadOnboardingState({
    ...
    conversationAutomationEnabled:
      automationScope.effectiveSettings.conversationAutomationEnabled,
  ...
  let liveProgramInventory: LiveProgramInventoryState = {
    ...
    message: capabilities.adsApiEnabled
      ? null
  ...
  if (capabilities.adsApiEnabled) {
    try {
      ...
      const currentUpstreamPrograms = sortProgramsByMostRecentActivity(
        upstreamPrograms.filter((program: (typeof upstreamPrograms)[number]) =>
          isCurrentUpstreamProgramStatus(program.program_status),
        ),
      );
...
function collectPartnerMigrationRecords(payload: unknown) {
  const root = asUnknownRecord(payload);
  const candidates: Array<{ key?: string; record: Record<string, unknown> }> =
    [];
  const directArrays = [
    payload,
    root.businesses,
    root.business_migration_infos,
    root.businessMigrationInfos,
    root.migration_infos,
    root.migrationInfo,
    root.results,
    root.data,
  ];
  ...
  for (const [key, value] of Object.entries(root)) {
    ...
    if (Object.keys(record).length > 0 && !Array.isArray(value)) {
      candidates.push({ key, record });
    }
  }

  const recordsByBusinessId = new Map<string, Record<string, unknown>>();

  for (const candidate of candidates) {
    const businessId =
      readStringValue(candidate.record, "yelp_business_id") ??
      readStringValue(candidate.record, "business_id") ??
      readStringValue(candidate.record, "source_business_id") ??
      readStringValue(candidate.record, "requested_business_id") ??
      readStringValue(candidate.record, "id") ??
      candidate.key;

    if (businessId) {
      recordsByBusinessId.set(businessId, candidate.record);
    }
  }
...
function buildBusinessSyncPatch(params: {
  status: ReadinessState["yelpBusinessSyncStatus"];
  checkedAt: string;
  source: string;
  destinationBusinessId?: string | null;
  destinationLocalBusinessId?: string | null;
  ...
  return {
    yelpBusinessSyncStatus: params.status,
    yelpBusinessSyncCheckedAt: params.checkedAt,
    yelpBusinessSyncSource: params.source,
    yelpBusinessSyncDestinationBusinessId: params.destinationBusinessId ?? null,
    yelpBusinessSyncDestinationLocalBusinessId:
      params.destinationLocalBusinessId ?? null,
    yelpBusinessSyncMergedBusinessIds: params.mergedBusinessIds ?? [],
    yelpBusinessSyncCorrelationId: params.correlationId ?? null,
    yelpBusinessSyncErrorCode: params.errorCode ?? null,
    yelpBusinessSyncErrorMessage: params.errorMessage ?? null,
  };
}

function summarizeBusinessSyncResults(
  source: string,
  results: Array<{
    businessId: string;
  ...
  return {
    ...
    noAccess: results.filter((result) => result.status === "NO_ACCESS").length,
    errors: results.filter((result) => result.status === "ERROR").length,
    results,
  };
}

async function runPartnerSupportBusinessSync(
  tenantId: string,
  actorId: string,
  businesses: Awaited<ReturnType<typeof listBusinesses>>,
  capabilities: Awaited<ReturnType<typeof getCapabilityFlags>>,
) {
  let credential:
    | Awaited<ReturnType<typeof ensureYelpAccess>>["credential"]
    | null = null;
  let lastCredentialError: unknown = null;
  const attempts = [
    {
      enabled:
        capabilities.businessMatchApiEnabled ||
        capabilities.hasPartnerSupportApi,
      capabilityKey: "businessMatchApiEnabled" as const,
      credentialKind: "BUSINESS_MATCH" as const,
    },
    {
      enabled: capabilities.dataIngestionApiEnabled,
      capabilityKey: "dataIngestionApiEnabled" as const,
      credentialKind: "DATA_INGESTION" as const,
    },
  ];
  ...
  if (!credential) {
    ...
    throw new YelpValidationError(
      "Enable Yelp Partner Support or Data Ingestion credentials before running migration sync.",
    );
  }

  const client = new YelpPartnerSupportClient(credential);
  const checkedAt = new Date().toISOString();
  const businessByYelpId = new Map(
    businesses.map((business) => [business.encryptedYelpBusinessId, business]),
  );
  const results: Array<{
    businessId: string;
    yelpBusinessId: string;
  ...
  for (
    let start = 0;
    start < businesses.length;
    start += YELP_BUSINESS_SYNC_BATCH_SIZE
  ) {
    const batch = businesses.slice(
      start,
      start + YELP_BUSINESS_SYNC_BATCH_SIZE,
    );
    const response = await client.getBusinessMigrationInfo(
      batch.map((business) => business.encryptedYelpBusinessId),
    );
    const migrationRecords = collectPartnerMigrationRecords(response.data);

    for (const business of batch) {
      const record = migrationRecords.get(business.encryptedYelpBusinessId);
      const migrationType = record
        ? readStringValue(record, "migration_type")
        : null;
      const destinationBusinessId = record
        ? (readStringValue(record, "destination_business_id") ??
          readStringValue(record, "destination_yelp_business_id"))
        : null;
      const mergedBusinessIds = record
        ? readStringArrayValue(record, "merged_business_ids")
        : [];
      const destinationLocalBusinessId = destinationBusinessId
        ? (businessByYelpId.get(destinationBusinessId)?.id ?? null)
        : null;
      const status: ReadinessState["yelpBusinessSyncStatus"] = !record
        ? "NOT_FOUND"
        : migrationType === "MIGRATED" || destinationBusinessId
          ? "MIGRATED"
          : "ACTIVE";
      const readinessJson = mergeReadinessPatch(
        business.readinessJson,
        buildBusinessSyncPatch({
          status,
          checkedAt,
          source: "PARTNER_SUPPORT_MIGRATION_INFO",
          destinationBusinessId,
          destinationLocalBusinessId,
          mergedBusinessIds,
  ...
  const summary = summarizeBusinessSyncResults(
    "PARTNER_SUPPORT_MIGRATION_INFO",
    results,
  );
  ...
  await recordAuditEvent({
    tenantId,
    actorId,
    actionType: "businesses.yelp-business-sync",
    status:
      summary.errors === summary.checked && summary.checked > 0
        ? "FAILED"
        : "SUCCESS",
    requestSummary: {
    ...
    responseSummary: summary as never,
  });

  return summary;
}

async function runLeadsProbeBusinessSync(
  tenantId: string,
  actorId: string,
  businesses: Awaited<ReturnType<typeof listBusinesses>>,
) {
  ...
  for (const business of businesses) {
    let status: ReadinessState["yelpBusinessSyncStatus"] = "ACTIVE";
    let correlationId: string | null = null;
    let errorCode: string | null = null;
    let errorMessage: string | null = null;

    try {
      const response = await client.getBusinessLeadIds(
        business.encryptedYelpBusinessId,
        { limit: 1, offset: 0 },
      );
      correlationId = response.correlationId;
      ...
      if (normalized.code === "NOT_FOUND") {
        status = "NOT_FOUND";
      } else if (normalized.code === "MISSING_ACCESS") {
        status = "NO_ACCESS";
      } else {
        status = "ERROR";
      }
  ...
  const summary = summarizeBusinessSyncResults("LEADS_BUSINESS_PROBE", results);
...
export async function runBusinessesYelpSync(tenantId: string, actorId: string) {
  const [businesses, capabilities] = await Promise.all([
  ...
  if (businesses.length === 0) {
    return summarizeBusinessSyncResults("NONE", []);
  }

  const canUsePartnerSupport =
    capabilities.businessMatchApiEnabled ||
    capabilities.hasPartnerSupportApi ||
    capabilities.dataIngestionApiEnabled;

  if (canUsePartnerSupport) {
    try {
      return await runPartnerSupportBusinessSync(
        tenantId,
        actorId,
        businesses,
        capabilities,
  ...
  if (!capabilities.hasLeadsApi) {
    throw new YelpValidationError(
      "Enable Yelp Partner Support or Yelp Leads credentials before syncing businesses.",
    );
  }

  return runLeadsProbeBusinessSync(tenantId, actorId, businesses);
}

export async function searchBusinessesForOnboarding(
  tenantId: string,
  input: unknown,
) {
      ...
      remoteState: {
        message:
          error instanceof Error
            ? error.message
            : "Business Match API is unavailable.",
      },
    };
  }
}
...
export async function saveBusinessRecord(
  ...
  const existing = await findBusinessByEncryptedYelpBusinessId(
    tenantId,
    match.encrypted_business_id,
  );
  const existingReadiness =
    typeof existing?.readinessJson === "object" &&
    existing.readinessJson !== null
      ? (existing.readinessJson as Record<string, unknown>)
      : {};
  const incomingReadiness =
    typeof match.readiness === "object" && match.readiness !== null
      ? match.readiness
      : {};
  ...
  await recordAuditEvent({
    ...
    businessId: business.id,
    actionType:
      match.source === "match" ? "business.match.save" : "business.manual.save",
    status: "SUCCESS",
...
export async function patchBusinessReadinessFields(
  tenantId: string,
  actorId: string,
  input: unknown,
) {
...
async function findYelpWebhookSubscription(
  ...
  for (let page = 0; page < 50; page += 1) {
    const response = await client.getBusinessSubscriptions(
      YELP_WEBHOOK_SUBSCRIPTION_TYPE,
      { limit, offset },
    );
    ...
    const match =
      subscriptions.find(
        (subscription) => subscription.business_id === encryptedYelpBusinessId,
      ) ?? null;
...
export async function runBusinessYelpLeadsReadinessCheck(
  tenantId: string,
  actorId: string,
  businessId: string,
) {
  ...
  if (!business.encryptedYelpBusinessId) {
    throw new YelpValidationError(
      "Save the Yelp encrypted business ID before checking Leads API access.",
    );
  }
...
export async function runBusinessYelpWebhookSubscriptionAction(
  ...
  if (!business.encryptedYelpBusinessId) {
    throw new YelpValidationError(
      "Save the Yelp encrypted business ID before managing webhook subscriptions.",
    );
  }
  ...
  if (data.action === "REQUEST_WEBHOOK") {
    try {
      ...
      return {
        ...
        status: "REQUESTED",
        message:
          "Yelp accepted the webhook subscription request. Check again after Yelp processes it.",
        correlationId: response.correlationId,
      };
    } catch (error) {
      const normalized = normalizeUnknownError(error);
  ...
  try {
    const result = await findYelpWebhookSubscription(
      client,
      business.encryptedYelpBusinessId,
    );
    const readinessJson = mergeReadinessPatch(business.readinessJson, {
      ...
      yelpLeadSubscriptionSubscribedAt:
        result.subscription?.subscribed_at ?? null,
...
const blockingBusinessDeletionStatuses = new Set([
  "ACTIVE",
  "SCHEDULED",
  "QUEUED",
  "PROCESSING",
...
export async function deleteBusinessWorkflow(
  ...
  const [business, deleteImpact] = await Promise.all([
    getBusinessById(data.businessId, tenantId),
    getBusinessDeleteImpact(data.businessId, tenantId),
  ]);
  const normalizedConfirmation = data.confirmationText.trim();

  if (normalizedConfirmation !== business.name) {
    throw new YelpValidationError(
      `Type the exact business name "${business.name}" to confirm deletion.`,
    );
  }

  const blockingPrograms = business.programs.filter((program) =>
    blockingBusinessDeletionStatuses.has(program.status),
  );
  ...
  try {
    ...
    if (result.count !== 1) {
      throw new YelpValidationError(
        "The selected business could not be deleted.",
      );
    }
