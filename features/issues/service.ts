...
function canRetryIssue(issue: {
  ...
  if (
    !getIssueStatusActionability(issue.status) ||
    !isRetryableIssueType(issue.issueType)
  ) {
    return false;
  }
...
function applyDeadLetterEscalation(
  candidate: DetectedIssue,
  detectedCount: number,
): DetectedIssue {
  if (
    !deadLetterIssueTypes.has(candidate.issueType) ||
    detectedCount < repeatedWorkerFailureDeadLetterThreshold
  ) {
    return candidate;
  }
  ...
  return {
    ...candidate,
    severity: "CRITICAL",
    title: candidate.title.startsWith("Repeated worker failure")
      ? candidate.title
      : `Repeated worker failure: ${candidate.title}`,
...
function summarizeErrors(
  rows: Array<{ message: string; code?: string | null }>,
) {
  return rows.map((row) =>
...
function buildLeadSyncDetectedIssues(
  ...
  return rows.map((run) => {
    ...
    const summary =
      run.errorSummary ??
      errorMessages[0] ??
      "Lead sync failed and needs review.";
...
function buildCrmSyncDetectedIssues(
  ...
  const deduped = latestByKey(
    rows,
    (run) =>
      run.leadId ? `crm-sync:lead:${run.leadId}` : `crm-sync:run:${run.id}`,
  ...
  return deduped.map((run) => {
    ...
    return {
      dedupeKey: run.leadId
        ? `crm-sync:lead:${run.leadId}`
        : `crm-sync:run:${run.id}`,
...
function buildStaleLifecycleDetectedIssues(
  ...
  return rows.map((mapping) => {
    const referenceAt =
      mapping.lastSyncedAt ?? mapping.matchedAt ?? mapping.updatedAt;
    ...
    return {
      ...
      locationId:
        mapping.locationId ??
        mapping.lead.locationId ??
        mapping.lead.business?.locationId ??
        null,
...
function buildMappingConflictDetectedIssues(
  ...
  return rows.map((mapping) => ({
    ...
    summary:
      mapping.issueSummary ?? "This lead has conflicting CRM mapping data.",
    businessId: mapping.lead.businessId ?? null,
    locationId:
      mapping.locationId ??
      mapping.lead.locationId ??
...
function buildAutoresponderDetectedIssues(
  ...
  return rows.map((attempt) => ({
    ...
    locationId:
      attempt.locationId ??
      attempt.lead.locationId ??
      attempt.business?.locationId ??
      null,
...
export async function refreshOperatorIssues(tenantId: string) {
  ...
  const candidates = [
    ...new Map(
      detectedCandidates.map((candidate) => [candidate.dedupeKey, candidate]),
    ).values(),
  ];
  const existingByKey = new Map(
    existingIssues.map((issue) => [issue.dedupeKey, issue]),
  );
  ...
  for (const candidate of candidates) {
    ...
    if (!existing) {
      try {
        const createdIssue = await createOperatorIssue(tenantId, {
        ...
        existingByKey.set(candidate.dedupeKey, createdIssue);
        createdCount += 1;
      } catch (error) {
        ...
        const racedIssue = await getOperatorIssueByDedupeKey(
          tenantId,
          candidate.dedupeKey,
        );
        ...
        existingByKey.set(candidate.dedupeKey, racedIssue);
      }
    ...
    const visibleCandidate = applyDeadLetterEscalation(
      candidate,
      nextDetectedCount,
    );
  ...
  const openCount =
    existingIssues.filter((issue) => issue.status === "OPEN").length +
    createdCount +
    reopenedCount -
    autoResolvedCount;
...
export async function refreshOperatorIssuesIfStale(
  tenantId: string,
  force = false,
) {
  const now = Date.now();
  ...
  if (!force) {
    if (
      ...
      now - (completedAt as number) < operatorIssueRefreshIntervalMs
    ) {
      return;
    }
    ...
    if (
      ...
      now - (startedAt as number) < operatorIssueRefreshLeaseMs
    ) {
      return;
    }
  }
...
export async function getOperatorQueue(
  tenantId: string,
  rawFilters?: OperatorIssueFiltersInput,
  queueOptions?: { refresh?: boolean },
) {
  if (queueOptions?.refresh !== false) {
  ...
  const totalPages =
    filteredTotal === 0 ? 1 : Math.ceil(filteredTotal / pageSize);
...
export async function getOperatorIssueDetail(
  tenantId: string,
  issueId: string,
) {
  await refreshOperatorIssuesIfStale(tenantId);
...
export async function retryOperatorIssueWorkflow(
  ...
  if (!canRetryIssue(issue)) {
    throw new YelpValidationError(
      "Retry is not available for this issue type.",
    );
  }
  ...
  try {
    if (issue.issueType === "LEAD_SYNC_FAILURE") {
      if (!issue.syncRunId) {
        throw new YelpValidationError(
          "This issue is not linked to a lead sync run.",
        );
      }
      ...
      if (!issue.syncRunId) {
        throw new YelpValidationError(
          "This issue is not linked to a downstream sync run.",
        );
      }
      ...
      if (!issue.reportScheduleRunId) {
        throw new YelpValidationError(
          "This issue is not linked to a report delivery run.",
        );
      }

      await resendReportScheduleRunWorkflow(
        tenantId,
        actorId,
        issue.reportScheduleRunId,
      );
      ...
      await retryLeadAutomationAttemptWorkflow(
        tenantId,
        actorId,
        issue.leadId,
        attemptId,
