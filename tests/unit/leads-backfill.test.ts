...
describe("lead backfill workflow", () => {
  beforeEach(() => {
    ...
    getLeadSyncRunById.mockImplementation(
      async (_tenantId: string, syncRunId: string) => {
    ...
    updateLeadSyncRun.mockImplementation(
      async (syncRunId: string, patch: Record<string, unknown>) => {
        if (backfillRunState && syncRunId === backfillRunState.id) {
          backfillRunState = {
            ...backfillRunState,
            ...patch,
    ...
    upsertLeadEventRecords.mockResolvedValue([{ id: "evt_local_1" }]);
  ...
  it("imports the returned Yelp lead IDs and preserves has_more on the sync run", async () => {
    ...
    const { syncBusinessLeadsWorkflow } =
      await import("@/features/leads/service");
    const result = await syncBusinessLeadsWorkflow("tenant_1", "user_1", {
      businessId: "business_1",
    });

    expect(getBusinessLeadIds).toHaveBeenNthCalledWith(
      1,
      "ys4FVTHxbSepIkvCLHYxCA",
      { limit: 20, offset: 0 },
    );
    expect(getBusinessLeadIds).toHaveBeenNthCalledWith(
      2,
  ...
  it("records per-lead failures without aborting the entire import", async () => {
    ...
    upsertLeadRecord.mockResolvedValueOnce({
      id: "local_lead_1",
    ...
    expect(getBusinessLeadIds).toHaveBeenCalledWith("ys4FVTHxbSepIkvCLHYxCA", {
      limit: 20,
      offset: 0,
    });
  ...
  it("queues raw webhook deliveries for async webhook-first processing", async () => {
    ...
    const result = await ingestYelpLeadWebhook(
      ...
      },
      {
        ...
        "x-irbis-forward-secret": "must-not-be-persisted",
  ...
  it("rejects an ambiguous Yelp business mapping instead of choosing a tenant", async () => {
    findBusinessesByExternalYelpBusinessId.mockResolvedValueOnce([
      {
        id: "business_1",
        tenantId: "tenant_1",
        locationId: "location_1",
    ...
    ).rejects.toThrow("mapped more than once");
  ...
  it("reconciles queued webhook deliveries by refreshing the lead snapshot from Yelp", async () => {
    ...
    const { reconcilePendingLeadWebhooks } =
      await import("@/features/leads/service");
    ...
    expect(processLeadAutoresponderForNewLead).toHaveBeenCalledWith(
      "tenant_1",
      "local_lead_1",
    );
      ...
      webhookEvents: [
        {
          id: "webhook_existing",
          eventKey:
            "leads_event:ys4FVTHxbSepIkvCLHYxCA:lead_existing:NEW_EVENT:evt_existing",
          payloadJson: {
            delivery: {
              time: "2026-04-01T09:00:00.000Z",
              object: "business",
  ...
  it("retries temporary webhook failures but skips non-retryable ones", async () => {
    ...
    listLeadWebhookSyncRunsForReconcile.mockResolvedValue([
      retryRun,
      skippedRun,
    ]);
    getLeadSyncRunById.mockImplementation(
  ...
  it("polls recent leads for autoresponder-enabled businesses and processes conversation automation", async () => {
    ...
    getLeadAutomationScopeConfig.mockImplementation(
      async (_tenantId: string, businessId: string) => ({
        effectiveSettings: {
    ...
    const { reconcileRecentYelpLeadsForAutomation } =
      await import("@/features/leads/service");
