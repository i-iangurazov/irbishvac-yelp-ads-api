...
beforeEach(() => {
  vi.clearAllMocks();
  reconcilePendingLeadWebhooks.mockResolvedValue([]);
});
...
describe("Yelp leads webhook route", () => {
  it("returns the verification token for Yelp webhook setup", async () => {
    ...
    const response = await GET(
      new Request(
        "http://localhost/api/webhooks/yelp/leads?verification=test-token",
      ),
    );
  ...
  it("accepts a webhook POST and queues the delivery for async processing", async () => {
    ...
    expect(reconcilePendingLeadWebhooks).toHaveBeenCalledWith(1);
  });
  ...
  it("fails closed in production when webhook authentication is not configured", async () => {
    vi.resetModules();
