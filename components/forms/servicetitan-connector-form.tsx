...
export function ServiceTitanConnectorForm({
  ...
  useEffect(() => {
    ...
    if (
      !values.apiBaseUrl ||
      Object.values(defaultUrls).some(
        (entry) => entry.apiBaseUrl === values.apiBaseUrl,
      )
    ) {
    ...
    if (
      !values.authBaseUrl ||
      Object.values(defaultUrls).some(
        (entry) => entry.authBaseUrl === values.authBaseUrl,
      )
    ) {
    ...
    try {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Unable to save ServiceTitan connector.",
      );
    }
  });
  ...
  const saveAndTest = async () => {
    ...
    try {
      ...
      const result = await apiFetch<{ message: string }>(
        "/api/integrations/servicetitan/test",
        {
          method: "POST",
        },
      );
      ...
      toast.error(
        error instanceof Error ? error.message : "ServiceTitan test failed.",
      );
    } finally {
      setIsTesting(false);
    }
  ...
  return (
    <Card className="shadow-none">
      <CardHeader>
        ...
        <CardDescription>
          Save connector credentials, tenant configuration, and environment in
          one place.
        </CardDescription>
      </CardHeader>
      <CardContent>
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setValue(
                    "environment",
                    value as ServiceTitanConnectorFormValues["environment"],
                  )
                }
              >
                <SelectTrigger>
          ...
          <Label className="flex items-center justify-between rounded-lg border border-border p-4">
            <div>
              ...
              <div className="text-sm text-muted-foreground">
                Turn off only if you need to pause live connector reads and
                reference-data sync.
              </div>
            </div>
            <Switch
