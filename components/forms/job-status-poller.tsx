...
export function JobStatusPoller({ jobId }: { jobId: string }) {
  ...
  const query = useQuery({
    ...
    refetchInterval: (queryData) =>
      queryData.state.data?.status === "QUEUED" ||
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Job polling hit an application error before the latest status could
            be shown.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-4 text-sm text-destructive">
            {query.error instanceof Error
              ? query.error.message
              : "Could not refresh the latest Yelp job status."}
          </div>
        </CardContent>
      </Card>
    );
  ...
  return (
    <Card className="overflow-hidden">
      <CardHeader>
        <CardTitle>Latest Yelp job</CardTitle>
        <CardDescription>
          Refreshes until Yelp returns a final job state.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4 overflow-hidden">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <span className="truncate text-sm text-muted-foreground">
            {query.data.upstreamJobId ?? "Local-only job"}
          </span>
        </div>
        {issue ? (
