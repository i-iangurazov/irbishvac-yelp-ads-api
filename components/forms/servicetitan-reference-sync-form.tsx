...
export function ServiceTitanReferenceSyncForm({
  ...
  const runSync = async (scope: "ALL" | "LOCATIONS" | "SERVICES") => {
    try {
      ...
      }>("/api/integrations/servicetitan/sync", {
        method: "POST",
      ...
      if (failures.length > 0) {
        toast.error(
          "ServiceTitan sync finished with failures. Check Audit for details.",
        );
        return;
      }
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Unable to sync ServiceTitan references.",
      );
    } finally {
  ...
  return (
    <div className="flex flex-wrap gap-3">
      <Button
        type="button"
        onClick={() => runSync("ALL")}
        disabled={disabled || Boolean(runningScope)}
      >
