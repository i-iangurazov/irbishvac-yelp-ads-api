...
export function LeadAutoresponderBusinessOverrideForm({
  ...
  const {
    ...
    defaultValues: {
      ...
      conversationAllowedIntents: initialValues?.conversationAllowedIntents ?? [
        "MISSING_DETAILS_PROVIDED",
        "BASIC_ACKNOWLEDGMENT",
        "SIMPLE_NEXT_STEP_CLARIFICATION",
      ],
      conversationMaxAutomatedTurns:
        initialValues?.conversationMaxAutomatedTurns ?? 2,
      conversationReviewFallbackEnabled:
        initialValues?.conversationReviewFallbackEnabled ?? true,
      conversationEscalateToIssueQueue:
        initialValues?.conversationEscalateToIssueQueue ?? true,
    },
  });
    ...
    try {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Unable to save the business override.",
      );
    }
  });
  ...
  const removeOverride = async () => {
    ...
    try {
      await apiFetch(
        `/api/settings/autoresponder/business-overrides/${businessId}`,
        {
          method: "DELETE",
        },
      );
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Unable to remove the business override.",
      );
    }
  };
  ...
  return (
    <Card className="shadow-none">
      <CardHeader className="pb-3">
        ...
        <CardDescription>
          Use an override only when one Yelp business needs a different live
          mode than the tenant default.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form className="space-y-6" onSubmit={submit}>
          <section className="space-y-4">
            <div>
              ...
              <div className="mt-1 text-xs text-muted-foreground">
                Choose the Yelp business first, then decide whether this
                override is live.
              </div>
            </div>
            <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_18rem] lg:items-end">
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setValue("businessId", value, { shouldValidate: true })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
              ...
              <div className="flex items-center justify-between rounded-lg border border-border/80 bg-muted/10 px-4 py-3">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    Use this override instead of the tenant default.
                  </div>
                </div>
                <Switch
                  checked={isEnabled}
          ...
          <section className="space-y-4 border-t border-border/70 pt-6">
            <div>
              ...
              <div className="mt-1 text-xs text-muted-foreground">
                Choose the primary response path and fallback behavior.
              </div>
            </div>
            <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,0.8fr)]">
              ...
              <div className="flex items-center justify-between rounded-lg border border-border/80 bg-muted/10 px-4 py-3">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    Only when thread delivery is unavailable.
                  </div>
                </div>
                <Switch
          ...
          <section className="space-y-4 border-t border-border/70 pt-6">
            <div>
              ...
              <div className="mt-1 text-xs text-muted-foreground">
                Controls AI-assisted live templates and review-mode lead tools
                for this business.
              </div>
            </div>
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      setValue(
                        "aiModel",
                        value as LeadAutoresponderBusinessOverrideValues["aiModel"],
                      )
                    }
                  >
                    <SelectTrigger>
          ...
          <section className="space-y-4 border-t border-border/70 pt-6">
            <div>
              ...
              <div className="mt-1 text-xs text-muted-foreground">
                Handle new inbound Yelp thread turns after the first response,
                without turning the thread into an open-ended bot.
              </div>
            </div>
            <div className="space-y-4">
              <div className="flex items-center justify-between rounded-lg border border-border/80 bg-muted/10 px-4 py-3">
                <div>
                  <div className="text-sm font-medium">Enabled</div>
                  <div className="text-xs text-muted-foreground">
                    Apply conversation handling for this Yelp business override.
                  </div>
                </div>
                <Switch
              ...
              {conversationAutomationEnabled ? (
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) =>
                          setValue(
                            "conversationMode",
                            value as LeadAutoresponderBusinessOverrideValues["conversationMode"],
                            {
                              shouldValidate: true,
                            },
                          )
                  ...
                  {conversationMode === "BOUNDED_AUTO_REPLY" ? (
                    <div className="space-y-3">
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">
                          Only these intent types may auto-send. Risky
                          categories still stop and hand off.
                        </div>
                      </div>
                      <div className="grid gap-3 lg:grid-cols-2">
                        {leadConversationIntentOptions
                          .slice(0, 3)
                          .map((intent) => {
                            const checked = conversationAllowedIntents.includes(
                              intent.value,
                            );
                  ...
                  <div className="grid gap-3 lg:grid-cols-2">
                    <div className="flex items-center justify-between rounded-lg border border-border/80 bg-muted/10 px-4 py-3">
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">
                          Use a suggested draft when bounded auto-reply stops
                          short of sending.
                        </div>
                      </div>
                      <Switch
                        ...
                        onCheckedChange={(checked) =>
                          setValue("conversationReviewFallbackEnabled", checked)
                        }
                      />
                    </div>
                    <div className="flex items-center justify-between rounded-lg border border-border/80 bg-muted/10 px-4 py-3">
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">
                          Create operator issues for blocked or risky
                          conversation turns.
                        </div>
                      </div>
                      <Switch
                        ...
                        onCheckedChange={(checked) =>
                          setValue("conversationEscalateToIssueQueue", checked)
                        }
                      />
                    </div>
                  </div>
                ...
                <div className="rounded-lg border border-dashed border-border/80 px-4 py-3 text-sm text-muted-foreground">
                  New inbound customer thread messages stay human-only for this
                  business.
                </div>
              )}
            </div>
          ...
          <section className="space-y-4 border-t border-border/70 pt-6">
            <div>
              ...
              <div className="mt-1 text-xs text-muted-foreground">
                Only send later nudges when the thread still has no safe stop
                condition.
              </div>
            </div>
            <div className="space-y-4">
              ...
              <div className="rounded-lg border border-border/80 p-4">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  <div className="max-w-2xl">
                    ...
                    <div className="mt-1 text-xs text-muted-foreground">
                      Runs later only when the thread is still eligible.
                    </div>
                  </div>
                  <Switch
