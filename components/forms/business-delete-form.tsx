    ...
    <div className="flex flex-col items-end gap-1">
      <AlertDialog open={open} onOpenChange={setOpen}>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              Delete this business from the console?
            </AlertDialogTitle>
            <AlertDialogDescription>
              This permanently removes the local business record and deletes all
              local programs, jobs, feature snapshots, and mappings tied to it.
              It does not send termination requests to Yelp, so active or
              pending programs must be resolved first.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="grid gap-3 rounded-lg border border-border bg-muted/30 p-4 text-sm">
            ...
            <div>
              Feature snapshots to delete: {deleteImpact.featureSnapshots}
            </div>
            <div>Business mappings to delete: {deleteImpact.mappings}</div>
            <div>
              Report requests that will be detached:{" "}
              {deleteImpact.reportRequests}
            </div>
            <div>
              Report results that will be detached: {deleteImpact.reportResults}
            </div>
            <div>
              Audit events that will be detached: {deleteImpact.auditEvents}
            </div>
          </div>
