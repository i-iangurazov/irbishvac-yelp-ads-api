...
export function ServiceTitanLifecycleSyncForm({
  ...
  const runSync = async (mode: "DUE" | "RECENT") => {
    try {
      setRunningMode(mode);
      const result = await apiFetch<LifecycleSyncResult>(
        "/api/integrations/servicetitan/lifecycle-sync",
        {
          method: "POST",
          body: JSON.stringify(
            mode === "RECENT"
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Unable to run ServiceTitan lifecycle sync.",
      );
    } finally {
      setRunningMode(null);
    }
  ...
  return (
    <div className="flex flex-wrap gap-3">
      <Button
        type="button"
        onClick={() => runSync("DUE")}
        disabled={disabled || Boolean(runningMode)}
      >
