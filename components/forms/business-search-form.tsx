...
type ResultState = {
  ...
  remote: Array<{
    ...
    state?: string;
    categories?: Array<
      string | { label?: string; alias?: string; title?: string; name?: string }
    >;
  }>;
...
export function BusinessSearchForm() {
  ...
  return (
    <div className="space-y-4">
      <form
        className="grid gap-4 rounded-lg border border-border/80 bg-card p-5 lg:grid-cols-[2fr_1fr_auto]"
        onSubmit={onSubmit}
      >
            ...
            <CardContent className="space-y-3 p-4">
              <div>
                ...
                <div className="text-xs text-muted-foreground">
                  Save the exact Yelp business the console should operate
                  against.
                </div>
              </div>
              {results.remoteState ? (
                <p className="text-sm text-warning">
                  {results.remoteState.message}
                </p>
              ) : null}
              <div className="space-y-3">
                {results.remote.map((match) => (
                  <div
                    key={match.encrypted_business_id}
                    className="rounded-lg border border-border p-3"
                  >
                    {(() => {
                      const categories = normalizeYelpCategories(
                        match.categories ?? [],
                      );
                      ...
                      return (
                        <>
                          ...
                          <div className="mt-2 text-sm">
                            {categories.length > 0
                              ? categories.map(formatYelpCategory).join(", ")
