...
export function ProgramForm(props: ProgramFormProps) {
  ...
  const defaultValues = {
    businessId:
      props.initialValues?.businessId ?? props.businesses[0]?.id ?? "",
  ...
  const {
    ...
    defaultValues:
      props.mode === "edit"
        ? { ...defaultValues, programId: props.programId }
        : defaultValues,
  });

  const selectedBusiness = props.businesses.find(
  ...
  const aliasBackedCategories = useMemo(
    () =>
      (selectedBusiness?.categories ?? []).filter(
        (category) =>
          typeof category.alias === "string" && category.alias.length > 0,
      ),
    [selectedBusiness],
  );
  ...
  const onSubmit = handleSubmit(async (values) => {
    try {
      const result =
        props.mode === "create"
          ? await apiFetch<{ programId: string; jobId: string }>(
              "/api/programs",
              {
                method: "POST",
      ...
      toast.success(
        props.mode === "create"
          ? "Program submitted to Yelp."
          : "Program update submitted to Yelp.",
      );
      router.push(`/programs/${result.programId}?jobId=${result.jobId}`);
      router.refresh();
    } catch (error) {
      toast.error(
        error instanceof Error ? error.message : "Unable to submit program.",
      );
    }
  });
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>
          {props.mode === "create" ? "Create program" : "Edit program"}
        </CardTitle>
        <CardDescription>
          Use dollar inputs. The console saves the request immediately, then
          waits for Yelp to confirm the final state.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form className="grid gap-5 lg:grid-cols-2" onSubmit={onSubmit}>
          <div className="space-y-2">
            ...
            {selectedBusiness?.readiness.isReadyForCpc === false &&
            programType === "CPC" ? (
              <p className="text-sm text-warning">
                CPC readiness issues:{" "}
                {selectedBusiness.readiness.missingItems.join("; ")}
              </p>
            ) : null}
                  ...
                  <Badge
                    variant={
                      eligibilityVariantMap[
                        selectedBusiness.readiness.adsEligibilityStatus
                      ]
                    }
                  >
                    {
          ...
          <div className="space-y-2">
              ...
              aliasBackedCategories.length > 0 ? (
                <div className="space-y-2 rounded-lg border border-border p-3">
                  <div className="rounded-md border border-border bg-muted/30 p-3 text-sm text-muted-foreground">
                    Leave all unchecked to omit{" "}
                    <span className="font-mono">ad_categories</span> and let
                    Yelp use the listing categories. Use checked aliases for a
                    more specific category CPC program alongside the main
                    listing-wide program.
                  </div>
                  {aliasBackedCategories.map((category) => (
                    <Label
                      key={category.alias}
                      className="flex items-start gap-3 rounded-md border border-transparent p-2 hover:bg-muted/40"
                    >
                      <Checkbox
                        checked={selectedCategoryAliases.includes(
                          category.alias!,
                        )}
                ...
                <div className="rounded-lg border border-border bg-muted/40 p-3 text-sm text-muted-foreground">
                  No Yelp category aliases are saved for this business yet. You
                  can still submit CPC without explicit{" "}
                  <span className="font-mono">ad_categories</span>.
                </div>
              )
            ) : (
              <div className="rounded-lg border border-border bg-muted/40 p-3 text-sm text-muted-foreground">
            ...
            {legacySelectedCategories.length > 0 ? (
              <p className="text-xs text-warning">
                Previously saved category values are not valid Yelp aliases and
                will be ignored.
              </p>
            ) : null}
            <p className="text-xs text-muted-foreground">
              Checked aliases are sent as Yelp{" "}
              <span className="font-mono">ad_categories</span>. Leave them
              unchecked to omit the field.
            </p>
            {errors.adCategories ? (
              <p className="text-sm text-destructive">
                {errors.adCategories.message as string}
              </p>
            ) : null}
          </div>
          ...
          <div className="space-y-2">
            <Label className="flex items-center gap-2">
              <Checkbox
                checked={isAutobid}
                onCheckedChange={(checked) =>
                  setValue("isAutobid", checked === true)
                }
              />
              Use Yelp autobid
            </Label>
            <p className="text-xs text-muted-foreground">
              If autobid is off, max bid is required.
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="22.00"
              disabled={isAutobid}
              {...register("maxBidDollars")}
            />
            {errors.maxBidDollars ? (
              <p className="text-sm text-destructive">
                {errors.maxBidDollars.message}
              </p>
            ) : null}
          </div>
          ...
          <div className="space-y-2">
            ...
            <Select
              defaultValue={watch("pacingMethod")}
              onValueChange={(value) =>
                setValue("pacingMethod", value as "paced" | "unpaced")
              }
            >
              <SelectTrigger>
                <SelectValue />
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setValue(
                    "feePeriod",
                    value as "CALENDAR_MONTH" | "ROLLING_MONTH",
                  )
                }
              >
                <SelectTrigger>
          ...
          <div className="space-y-2">
            <Label htmlFor="scheduledBudgetEffectiveDate">
              Future budget change date
            </Label>
            <Input
              id="scheduledBudgetEffectiveDate"
              type="date"
              disabled={props.mode === "create"}
              {...register("scheduledBudgetEffectiveDate")}
            />
            {props.mode === "create" ? (
              <p className="text-xs text-muted-foreground">
                Future budget changes are documented on program edit after the
          ...
          <div className="space-y-2 lg:col-span-2">
            ...
            <Textarea
              id="notes"
              placeholder="Explain why this request is being submitted."
              {...register("notes")}
            />
          </div>
          ...
          <div className="rounded-lg border border-border bg-muted/40 p-4 lg:col-span-2">
            ...
            <div className="mt-2 grid gap-2 text-sm text-muted-foreground md:grid-cols-2">
              <div>
                ...
                {
                  programTypeLabels[
                    programType as keyof typeof programTypeLabels
                  ]
                }
              </div>
              <div>
